...
static inline HANDLE winFindFirstFileW(const WCHAR *path, LPWIN32_FIND_DATA d)
{
...
static inline HANDLE winCreateFileW(const WCHAR *wfname, const DWORD mode, const DWORD creation)
{
    ...
    return CreateFile(wfname, mode, share, NULL, creation, FILE_ATTRIBUTE_NORMAL, NULL);
} /* winCreateFileW */

static BOOL winSetFilePointer(HANDLE h, const PHYSFS_sint64 pos, PHYSFS_sint64 *_newpos, const DWORD whence)
{
...
char *__PHYSFS_platformCalcBaseDir(const char *argv0)
{
    ...
    while (1)
    {
        ...
        modpath = (LPWSTR) ptr;
        rc = GetModuleFileName(NULL, modpath, buflen);
        if (rc == 0)
        {
            allocator.Free(modpath);
...
char *__PHYSFS_platformCalcPrefDir(const char *org, const char *app)
{
    ...
    if (!SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_APPDATA | CSIDL_FLAG_CREATE, NULL, 0, path))) BAIL(PHYSFS_ERR_OS_ERROR, NULL);
...
static int doPlatformDelete(LPWSTR wpath)
{
    ...
    if (!GetFileAttributesEx(wpath, GetFileExInfoStandard, &info))
        BAIL(errcodeFromWinApi(), 0);
    else
    {
        const int isdir = (info.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY);
        const BOOL rc = isdir ? RemoveDirectory(wpath) : DeleteFile(wpath);
...
int __PHYSFS_platformStat(const char *filename, PHYSFS_Stat *st, const int follow)
{
    ...
    rc = GetFileAttributesEx(wstr, GetFileExInfoStandard, &winstat);
    ...
    else if (winstat.dwFileAttributes & (FILE_ATTRIBUTE_DEVICE))
    {
